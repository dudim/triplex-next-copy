...
- Локальные тесты могут использоваться по необходимости, но не являются обязательной частью CI
- Для PR в GitHub обязательным check является сборка библиотеки
